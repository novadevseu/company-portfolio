...
export const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (manualNavigation) return; // Si se hizo clic en un enlace, no actualizamos el hash con scroll.
    ...
    handleScroll(); // Llamamos la función al inicio para actualizar el hash correctamente
    ...
    <>
      <nav className="w-full h-20 flex flex-col justify-center items-center fixed bg-background lg:bg-transparent z-40 lg:backdrop-blur-xl">
        <div className="2xl:w-[1280px] xl:w-10/12 w-11/12 flex justify-between items-center relative">
              ...
              {navbarLinks.map(({ href, label, id }) => (
                <a
                  ...
                  onClick={() => {
                    ...
                    setTimeout(() => setManualNavigation(false), 1000); // Permitir la actualización dinámica después de 1s
                  }}
                >
                  {label}
                  {active === id && (
          ...
          {/* Mobile Menu Button */}
          <div
            className="lg:hidden flex flex-col px-2 py-3 border border-transparent rounded-md cursor-pointer hover:bg-accent transition"
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
            >
              <div className="flex flex-col mt-16 lg:hidden absolute top-4 left-0 bg-background z-50 w-full items-center gap-10 pb-10 pt-10">
